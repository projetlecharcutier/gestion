...
- `rebuildBuildingGrid()` construit la grille spatiale O(1) des bâtiments puis **recalcule le flow field** `rebuildNavGrid()` (BFS vers la ville + flèches par cellule, `G.NAV_CELL` = 32 px, `src/flowfield.js`) consommé par les zombies (`G.navStep`/`G.navAngle`, voir `docs/zombies.md`).
- `ensureForetConnectivity()` (`src/flowfield.js`) retire les forêts qui referment des enclaves libres inaccessibles depuis la ville (appelé en fin de `buildWorld` et après repousse).
