...
- **Flow field** (`G.rebuildNavGrid`/`G.navStep`/`G.navAngle`, `src/flowfield.js`) : BFS multi-sources depuis le périmètre de la ville sur une grille grossière (`G.NAV_CELL` = 32 px, marge 8 px) recalculé à chaque `rebuildBuildingGrid()`, puis **flèche précalculée par cellule** (`G.navDirX`/`G.navDirY`, vecteur unitaire vers le voisin 8-connexe de distance BFS minimale). Le cap de base du chef suit la flèche (`navAngle`, O(1) par tick) — il contourne les massifs par le chemin le plus court. Les membres bloqués par une forêt consultent la flèche de leur cellule (`navStep`) avant l'évasion locale : la horde entière suit la même carte. Les coupes de forêts ouvrent automatiquement de nouveaux passages (rebuild au chop).
...
- **Répartition sur les 4 murs** : le champ BFS descend tous les gradients vers le point de couronne le plus proche — un ANGLE de la ville pour un groupe qui arrive en diagonale. Chaque groupe porte donc un mur assigné (`navSide` : 0=nord, 1=sud, 2=ouest, 3=est, -1=le plus proche ; `navT` : abscisse normalisée sur le mur, tirée au hasard) et son chef vise le point de ce mur (`navTargetOf`) tant qu'il est à plus de 30 px : les hordes se répandent sur TOUT le périmètre au lieu de s'empiler sur les coins. La flèche du champ ne guide le chef que si l'objectif est à plus de `ZOMBIE_NAV_DIRECT_DIST` px ; plus près, cap direct sur le mur assigné.
- **Manœuvre anti-blocage** : un chef ou un membre qui ne progresse plus (la distance à son objectif ne baisse pas, le glissement le long d'un obstacle ne compte pas comme un progrès) pendant `ZOMBIE_UNSTICK_TIME` secondes et qui n'est PAS à proximité de la ville (marge `ZOMBIE_UNSTICK_TOWN` : près de la palissade un zombie bloqué doit la presser, pas reculer) déclenche une manœuvre de dégagement : `ZOMBIE_UNSTICK_BACK` secondes cap opposé au blocage, puis `ZOMBIE_UNSTICK_SIDE` secondes à 90°, puis reprise de la navigation normale (cooldown `ZOMBIE_UNSTICK_CD`). L'état vit sur l'objet (`grp.unstick`/`z.unstick`), la direction de fuite est mémorisée (`base`/`side`).
